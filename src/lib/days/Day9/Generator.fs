...
let Generate (rnd: System.Random) ``number of entries`` (``window size``: int) =
    ...
    let unfolder (state: bigint list) =
        ...
        let index2 = inner ()
        let nextNumber = state.[index] + state.[index2]

        Some
            ((nextNumber, [| index; index2 |]),
             nextNumber
             :: List.take (List.length state - 1) state)

    let initialNumbers =
        List.init ``window size`` (fun _ -> rnd.Next(1, ``window size`` / 5) |> bigint)

    let noErrors =
        Seq.append (initialNumbers |> Seq.map (fun i -> i, [||])) (initialNumbers |> List.rev |> Seq.unfold unfolder)
    ...
    let rec getBlockSum () =
        ...
        if Seq.contains blockSum avoidValues then getBlockSum () else blockSum

    noErrors
    |> Seq.mapi (fun i (v, _) ->
        (if i = replaceIndex then getBlockSum () else v)
        |> string)
    |> String.concat "\n"

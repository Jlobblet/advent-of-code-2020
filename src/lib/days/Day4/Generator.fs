...
let Generate (rnd: Random) ``number of entries to generate`` =
    let rndInt lower upper = rnd.Next(lower, upper)
    ...
    let generatePassport () =
        ...
        let hcl =
            ...
            | l when l <= 0.5 ->
                Some
                    (sprintf "#%s"
                     <| String.concat "" (List.init 6 (fun _ -> string <| List.item (rndInt 0 15) hexDigit)))
            | m when m <= 0.6 ->
                Some(String.concat "" (List.init 6 (fun _ -> string <| List.item (rndInt 0 15) hexDigit)))
            | h when h <= 0.7 ->
                Some
                    (sprintf "#%s"
                     <| String.concat "" (List.init (rndInt 4 8) (fun _ -> string <| List.item (rndInt 0 15) hexDigit)))
        ...
        let pid =
            ...
            | l when l <= 0.7 ->
                Some
                    (List.init 9 (fun _ -> string <| rndDigit ())
                     |> List.reduce (+))
            | m when m <= 0.8 ->
                Some
                    (List.init (rndInt 5 15) (fun _ -> string <| rndDigit ())
                     |> List.reduce (+))
            | h when h <= 0.9 ->
                Some
                    (List.init (rndInt 5 15) (fun _ -> string <| rndLet ())
                     |> List.reduce (+))
            | _ -> None

...
let (|GetInput|) =
    File.ReadAllLines >> Array.map (int >> bigint)
...
let SolutionB (timer: Timer) (GetInput input) =
    ...
    input |> Array.sort |> Array.append [| 0I |]
    |!> timer.Lap "Sorting"
    |> List.ofArray
    |> List.pairwise
...
let SolutionBF (timer: Timer) (GetInput input) =
    ...
    |> Array.fold (fun acc elt ->
        acc
        |> Map.change elt (fun _ -> Some(acc.[elt + 1I] + acc.[elt + 2I] + acc.[elt + 3I]))) map
    |!> timer.Lap "Folding"
    |> Map.find 0I
    |> string
